...
class TextEncoder(nn.Module):
    ...
    def forward(
        ...
        if padding_mask is not None:
            ...
            new_padding_mask = torch.zeros_like(
                key_padding_mask, dtype=x.dtype)
            padding_mask = new_padding_mask.masked_fill(
...
class AudioEncoder(nn.Module):
    ...
    def forward(
        ...
        self.input_positional_embedding = self.positional_embedding[:src_len]
        assert x.shape[1:
                       ] == self.input_positional_embedding.shape, f"incorrect audio shape: {x.shape[1:], self.input_positional_embedding.shape}"
        x = (x + self.input_positional_embedding).to(x.dtype)
        if padding_mask is not None:
