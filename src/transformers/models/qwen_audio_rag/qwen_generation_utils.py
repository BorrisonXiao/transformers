    ...
    if chat_format == "chatml":
        ...
        def _tokenize_str(role, content):
            ...
            return f"{role}\n{content}", tokenizer.encode(
                role, allowed_special=set(tokenizer.AUDIO_ST).union(set(tokenizer.EXTRAS)), audio_info=audio_info
            ) + nl_tokens + tokenizer.encode(content, allowed_special=set(tokenizer.AUDIO_ST).union(set(tokenizer.EXTRAS)), audio_info=audio_info), audio_info

        system_text, system_tokens_part, audio_info = _tokenize_str(
            "system", system)
        ...
        for turn_query, turn_response in reversed(history):
            query_text, query_tokens_part, _ = _tokenize_str(
                "user", turn_query)
        ...
        if labels_str is not None:
            context_tokens += (
                tokenizer.encode(labels_str, allowed_special=set(tokenizer.AUDIO_ST).union(set(tokenizer.EXTRAS)))
                + im_end_tokens
            )
            raw_text += f"{labels_str}{im_end}"
...
def _decode_default(
    ...
    *,
    ...
    trim_decode_tokens = tokenizer.decode(
        tokens, errors=errors, **kwargs)[raw_text_len:]
    if verbose:
...
def _decode_chatml(
    ...
    *,
    ...
    trim_decode_tokens = tokenizer.decode(
        tokens[:eod_token_idx], errors=errors, **kwargs)[raw_text_len:]
    if verbose:
        print("\nRaw Generate w/o EOD:", tokenizer.decode(tokens,
              errors=errors, **kwargs)[raw_text_len:])
        print("\nRaw Generate:", trim_decode_tokens)
        print("\nEnd Reason:", end_reason)
    for stop_word in stop_words:
        trim_decode_tokens = trim_decode_tokens.replace(stop_word, "").strip()
