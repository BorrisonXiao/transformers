...
"""SpeechMult5 model configuration"""
...
class SpeechMult5Config(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`SpeechMult5Model`]. It is used to instantiate a
    SpeechMult5 model according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of the SpeechMult5
    [microsoft/speechmult5_asr](https://huggingface.co/microsoft/speechmult5_asr) architecture.
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 81):
            Vocabulary size of the SpeechMult5 model. Defines the number of different tokens that can be represented by
            the `inputs_ids` passed to the forward method of [`SpeechMult5Model`].
        ...
        num_mel_bins (`int`, *optional*, defaults to 80):
            ...
            the value used in the [`SpeechMult5Processor`] class.
        ...
        guided_attention_text_scale (`float`, *optional*, defaults to 1.0):
            Relative weight for the text diagonal term inside the unscaled guided attention loss.
        use_cache (`bool`, *optional*, defaults to `True`):
            Whether or not the model should return the last key/values attentions (not used by all models).
        sync_matrix_dropout (`float`, *optional*, defaults to 0.0):
            The dropout probability for the sync matrix. If 1.0, the sync matrix is completely dropped for all samples.
    ...
    ```python
    >>> from transformers import SpeechMult5Model, SpeechMult5Config

    >>> # Initializing a "microsoft/speechmult5_asr" style configuration
    >>> configuration = SpeechMult5Config()

    >>> # Initializing a model (with random weights) from the "microsoft/speechmult5_asr" style configuration
    >>> model = SpeechMult5Model(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config
    ```"""

    model_type = "speechmult5"
    ...
    def __init__(
        ...
        guided_attention_loss_num_heads=8,
        ...
        **kwargs,
        ...
        self.guided_attention_text_scale = guided_attention_text_scale

        self.use_cache = use_cache
        self.is_encoder_decoder = is_encoder_decoder
        self.sync_matrix_len = sync_matrix_len
        self.sync_matrix_dropout = sync_matrix_dropout
        self.default_decoder_prefix_ids = (
            list(default_decoder_prefix_ids)
...
class SpeechMult5HifiGanConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`SpeechMult5HifiGanModel`]. It is used to instantiate
    a SpeechMult5 HiFi-GAN vocoder model according to the specified arguments, defining the model architecture.
    Instantiating a configuration with the defaults will yield a similar configuration to that of the SpeechMult5
    [microsoft/speechmult5_hifigan](https://huggingface.co/microsoft/speechmult5_hifigan) architecture.
    ...
    ```python
    >>> from transformers import SpeechMult5HifiGan, SpeechMult5HifiGanConfig

    >>> # Initializing a "microsoft/speechmult5_hifigan" style configuration
    >>> configuration = SpeechMult5HifiGanConfig()

    >>> # Initializing a model (with random weights) from the "microsoft/speechmult5_hifigan" style configuration
    >>> model = SpeechMult5HifiGan(configuration)
